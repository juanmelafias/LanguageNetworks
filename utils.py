...
def plotly_graph(G,colors,palabras,display_legend,picsdir):
	nodeList = [int(node) for node in G.nodes()]
	edge_x = []
	edge_y = []
	pos = nx.spring_layout(G)
	colordict = {1:'blue',2:'yellow',3:'green',4:'red',5:'violet'}
	...
	if len(set(colors)) == 2:
		...
		legendgraph = {'blue':'OuterPeriphery','yellow':'InnerPeriphery', 'green':'SuperCore','red':'InerConnectors','violet':'OuterConnectors'}
	...
	if not os.path.exists(finaldir):
		os.mkdir(finaldir)
	fig.write_image(f'{finaldir}/nc{len(set(colors))}.png',engine='orca')'''