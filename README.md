...
This repository provides the tools to create Syntax Language Networks from syntactically annotated corpora. Unlike adjacency networks, which connect words based on their proximity in a sentence, syntactical networks are constructed using syntactic relationships between words. Additionally, the repository implements a novel method for network vector representation, enabling the study of syntactical similarities across languages. 

While the framework is designed to work with any language, it includes examples and data for specific languages to facilitate experimentation and analysis.
...
- **POS**: Part of Speech or grammatical class of each word. Useful to filter/represent symbols.
...
- **lemmatized**: True or False depending on whether the network is built on lemmatized networks or inflected ones.
...
- **rgb1, rgb2, rgb3**: Normalized principal components to fit the RGB scale such that each node can be colored with (rgb1, rgb2, rgb3).
- **pc1is, pc2is, pc3is**: First three principal components in the inflected Spanish eigenspace. Useful to compare words across languages.
- **nc2, nc3, nc4, nc5**: Color codes for 2, 3, 4, and 5 hierarchical clusters.
- **trans**: Translation to English of the word. "no trans" if translation to that language is not provided by Google.
- **ranking_inv**: The inverse rank described above. 501-ranking. Useful to use as the size of markers, as more frequent words appear bigger.
- **Docker**
...
## Scripts
This folder stores a series of python scripts useful for Network Analisys taken from brigan/NodeMorphospaces Github repository, along with the ***helper.py*** and ***loadHelper.py*** Scripts.
